...
resource "aws_ssm_parameter" "postgresql_major_version" {
  ...
  type        = "SecureString"
  key_id      = data.aws_kms_key.ssm.id
  value       = split(".", module.database.engine_version_action)[0] # The install script only needs the major version number.
}
...
resource "aws_ssm_parameter" "tfe_database_password" {
  ...
  type        = "SecureString"
  key_id      = data.aws_kms_key.ssm.id
  value       = random_string.tfe_database_password
}
...
resource "aws_ssm_parameter" "tfe_object_storage_s3_region" {
  ...
  type        = "SecureString"
  key_id      = data.aws_kms_key.ssm.id
  value       = module.s3_storage.aws_region
}
